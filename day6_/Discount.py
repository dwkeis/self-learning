...
class Price:
    ...
    def product(self):
        ...
        if self.what == 'milk':
            money = Milk().output(self.discount_type.calculate(self.value,self.discount)) 
            return money
        if self.what == 'coffee':
            money = Coffee().output(self.discount_type.calculate(self.value,self.discount))
            return money
class Drink:
    # class of drink
    def price(self):
